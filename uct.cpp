...
int UCT::select_children_list_index(Tree_node* cur_node)
{
	...
	for (int i=0; i<child_num_; i++) {
		...
		float exploitation = (float)child_node.score / (child_node.num_visit + 0.1);
		float exploration = this->exploration_param * sqrt( log((float)cur_node->num_visit + this->t_param) / (child_node.num_visit + 0.1));
